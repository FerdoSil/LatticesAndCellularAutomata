          ...
          congr' 2; simp [eq₁, eq₂],
            {
              ...
              norm_cast, rw nat.div_div_eq_div_mul, rw mul_comm c r,
              simp[@nat.div_eq_of_lt n (r * c) nltcr],
              norm_cast,
            ...
            {
              rw length_generate_eq_size, simp [size, cols, rows, eq₁, eq₂],
              rw [add_comm],
              have h₄ : |↑n / ↑c| < r, by norm_cast at *; exact h₃,
              have h₅ : |↑n % ↑c| < c,
                begin
                  rw [← int.coe_nat_lt_coe_nat_iff, int.nat_abs_of_nonneg],
                  apply @int.mod_lt_of_pos ↑n ↑c (by norm_cast; exact cpos),
                  have cnezero : c ≠ 0, by intros contra; rw contra at cpos; linarith,
