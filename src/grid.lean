...
lemma gtry_lt_bly : (gtr g).y < (bl g).y :=
  expand_gtr g ▸ sub_lt_iff_lt_add.2 (lt_add_of_pos_right _ coe_rows_pos)
...
def relpoint_of_gpoint {g : α} (p : grid_point g) : relative_point g :=
    ⟨
      ⟨|p.x.1 - (tl g).y|,
       have h : p.x.1 - (tl g).y ≥ 0,
         by let lbx := p.x.2.1; rw tly_eq_try; linarith,
...
begin
  ...
  have : x - (tl g).y ≥ 0, by rw tly_eq_try; linarith,
  have : y - (tl g).x ≥ 0, by rw blx_eq_tlx.symm; linarith,
...
def rel_point_to_fin {α : Type} [grid α] {g : α}
  ...
  ⟨p.x * relative_grid.cols g + p.y, linearize_array p.y.2 p.x.2⟩

def grid_point_to_fin {α : Type} [grid α] {g : α}
  (p : grid_point g) : fin (size g) := rel_point_to_fin (relpoint_of_gpoint p)

lemma grid_point_to_fin_eq {α : Type} [grid α] {g : α}
  (p : grid_point g) : grid_point_to_fin p =
  ⟨|p.x.1 - (tl g).y| * relative_grid.cols g + |p.y.1 - (tl g).x|,
  linearize_array
    begin 
    ...
    begin
      ...
      simp [expand_gtr] at xl,
      simp only [tl], rw ← int.coe_nat_lt_coe_nat_iff,
      have : x - ((grid.bl g).y - ↑(relative_grid.rows g)) ≥ 0, by linarith,
      rw int.nat_abs_of_nonneg this,
      linarith
    end⟩ :=
...
private lemma absolute_bounds {o : ℤ} {r : ℕ}
                              (x : fin r) : o - ↑r + ↑x < o :=
  by simp; rw [← sub_eq_add_neg, sub_lt_zero]; cases x; finish
  ...
  relative_grid (fgrid α) := {
    ...
    g.data ⟨g.o.y - ↑g.r + x,
           ...
           ⟨g.o.x + y,
            ⟨le_add_of_nonneg_right $ by cases x; unfold_coes,
             add_lt_add_left (
               iff.elim_right int.coe_nat_lt y.2
            ) g.o.x⟩⟩
}
...
variables {α : Type} {ag : agrid₀ α} {fg : fgrid α}
          ...
          use h, split,
            ...
            },
            {
              ...
              have h₅ : repeat (tr'.y) ( |tr'.x - bl.x| ) =
                ...
                {
                  rw repeat_more, simp [ge_from_le],
...
lemma abs_data_eq_nth_a' {α : Type} {g : agrid₀ α} {p} :
  ...
  by simp [
       abs_data, (∘), relpoint_of_gpoint, prod_of_rel_point, uncurry, data,
       grid_point_to_fin, rel_point_to_fin, tl, bl, rows, cols
     ]
...
begin
  ...
  have i_bounded : i < g₁r * g₁c,
    {
      have yb : y - tlx < ↑g₁c, from sub_lt_iff_lt_add'.2 yu,
      have xb : x - tly < ↑g₁r, by rw [← sub_add, add_comm]; linarith,
      rw hi,
...
def row (n : fin (relative_grid.rows g)) :
  (fin (relative_grid.cols g)) → relative_grid.carrier α :=
  relative_grid.data g n

def col (n : fin (relative_grid.cols g)) :
  (fin (relative_grid.rows g)) → relative_grid.carrier α :=
  flip (relative_grid.data g) n

def top :=
  row g ⟨
    0,
    and.elim_left (gt_and_gt_of_mul_gt (relative_grid.unempty g))
  ⟩
...
def right :=
  have h : relative_grid.cols g > 0,
    from (gt_and_gt_of_mul_gt (relative_grid.unempty g)).2,
  col g ⟨nat.pred (relative_grid.cols g), nat.pred_lt (ne_of_gt h)⟩
...
theorem grid_eq_ext_a_a {α : Type} {g₁ g₂ : agrid₀ α}
  (hrows : g₁.r = g₂.r)
  (hcols : g₁.c = g₂.c)
  (horig : g₁.o = g₂.o) : ℘ g₁ = ℘ g₂ → g₁ = g₂ :=
...
attribute [extensionality]
theorem grid_eq_ext_a_f {α : Type} {g₁ : agrid₀ α} {g₂ : fgrid α}
  (hrows : g₁.r = g₂.r)
  (hcols : g₁.c = g₂.c)
  (horig : g₁.o = g₂.o) : ℘ g₁ = ℘ g₂ → g₁ = g₂ :=
...
attribute [extensionality]
theorem grid_eq_ext_f_a {α : Type} {g₁ : fgrid α} {g₂ : agrid₀ α}
  (hrows : g₁.r = g₂.r)
  (hcols : g₁.c = g₂.c)
  (horig : g₁.o = g₂.o) : ℘ g₁ = ℘ g₂ → g₁ = g₂ :=
